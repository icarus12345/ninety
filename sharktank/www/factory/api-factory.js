function APIFactory(
            ...
            if(opt.showerror!==false){
                ...
                switch(res.status){
                    ...
                    case 500:
                        ...
                        break;
                    default:
                        message = 'Oops, Server is busy.';
                }
                Dialog.error(message || 'Oops, Server is busy, please try again',_run);
            }
    ...
    $rootScope.show_terms = function(){
        var term = StorageService.get('term');
        if(term){ 
                $rootScope.terms_condition = (term);
        }else{
            if(!$rootScope.terms_condition){
                $http({
                    // method: 'JSONP',
                    ...
                    $rootScope.terms_condition = res.data.data;
                    StorageService.set('term', '',res.data.data);
                },function(res, status,headers){
                    Dialog.error('Fail to load term and condition');
                })
            }
        }
        SharedState.turnOn('IsShowTerms');
    }
