...
function Category(opt){
    ...
    this.tutorial = opt.tutorial || undefined;
    ...
    this.init = function(){
        try {
            ...
            if(this.count_question>0){
                ...
                this.processNum = Math.round(this.process*5)
            }
        } catch (e){
    ...
    this.get_answered_num = function(){
        return this.questions.reduce(function(sum, q) {
                return sum + (q.answered[0]>=0?1:0); 
            }, 0)
        ;
    }
}
function CategoryFactory(
    ...
    var me = this;
    this.getTitleFont = function(html){
        ...
        pElmDom.style['fontWeight'] = 'bold';
        pElmDom.className = 'question-title sf-bold';
        document.body.appendChild(pElmDom);
        var elm = document.createElement('div');
        pElmDom.appendChild(elm);
        elm.innerHTML = html
        var fontSize = 1;
        var pheight = pElmDom.clientHeight;
        var maxFontSize = pheight/2;
        while(true && fontSize<=maxFontSize){
            pElmDom.style.fontSize = fontSize + 'px';
            // elm.style.lineHeight = Math.floor(pheight) + 'px';
            if(elm.clientHeight<pElmDom.clientHeight) fontSize++;
            else break;
        }
        pElmDom.remove();
        return fontSize - 1;
    }
    this.getContentFont = function(html){
        var pElmDom = document.createElement('div');
        pElmDom.style.opacity = '0.01';
        pElmDom.style.overflow = 'hidden';
        pElmDom.style['position'] = 'absolute';
        ...
        pElmDom.style['left'] = '0px';
        pElmDom.style['zZndex'] = '-1';
        pElmDom.className = 'question-content sf-semibold';
        document.body.appendChild(pElmDom);
        var elm = document.createElement('div');
        pElmDom.appendChild(elm);
        elm.innerHTML = html
    ...
    this.build = function(data){
        // try {
            ...
            for(var i = 0; i<questions.length;i++){
                ...
                // q.title_fz = this.getTitleFont(q.title);
                // q.question_fz = this.getContentFont(q.question);
    ...
    this.get = function(id, callback){
        ...
        if(!this._categories){
            ...
            //     data = me._categories[id];
            //     if(typeof callback == 'function') callback(data);
            // })
            Helper.alert('Category not yet load.')
        } else {
            if(id==0) id = this.rootId;
            data = this._categories[id];
            if(typeof callback == 'function') callback(data);
        }
        return data;
    }
...
APP.factory('Category', CategoryFactory)