...
class Core_Model extends CI_Model {
    ...
    function _buildTree(array $elements, $parent_id = 0,$parents=array(0)) {
        $branch = array();
        foreach ($elements as $element) {
            if ($element->parent_id == $parent_id) {
                $tmp=$parents;$tmp[]=$element->id;
                $children = $this->_buildTree($elements, $element->id,$tmp);
                if ($children) {
    ...
    function _buildTreeArray(array $elements, $opt = null) {
        if(empty($opt)){
            $opt = array(
            'parent_id'=>0,
            'level'=> 0,
        ...
        if($parent_id==0){
            for ($i=0;$i<count($elements);$i++) {
                ...
                for ($j=0;$j<count($elements);$j++) {
                    if( 
                        $elements[$i]->parent_id==$elements[$j]->{$prefix.'id'}
                    ){
                        $f=true;
                        break;
                    }
                ...
                if($elements[$i]->parent_id == $elements[$i]->{$prefix.'id'}){
                    $elements[$i]->parent_id=0;
                    $elements[$i]->parent_title='';
                    $elements[$i]->path='';
                    $elements[$i]->error=2;
                }
                $elements[$i]->_value=">".$elements[$i]->{$prefix.'id'};
            }
        }
        $branch = array();
        foreach ($elements as $element) {
            if ($element->parent_id == $opt['parent_id']) {
                $element->level = $level;
                $element->parent_title=$parent_title;
                $element->display=repeater('----',$level).$element->{$prefix.'title'};
                $element->display=$path."/".$element->{$prefix.'title'};
                $element->_value=$value.">".$element->{$prefix.'id'};
                $element->isparent = 0;
                $children = $this->_buildTreeArray($elements, array(
                    'parent_id'=> $element->{$prefix.'id'},
                    'level'=> $element->level+1,
                    'parent_title'=> $element->{$prefix.'title'},
                    'path'=> $path.'/'.$element->{$prefix.'title'},
                    'value'=> $value.">".$element->{$prefix.'id'},
                    ));
    ...
    function _updateBatch($aaData){
        if(!empty($aaData)) foreach ($aaData as $c){
            if($c->value!=$c->_value){
                $this->onUpdate($c->{$this->prefix.'id'},array("{$this->prefix}value"=>$c->_value));
            }
        }
    }
}