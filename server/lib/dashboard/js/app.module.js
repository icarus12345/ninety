    ...
    App.Module = (function(){
        ...
        var columns = [
            {
                ...
                text: 'Group', dataField: 'grouptitle', minWidth: 180, sortable: false,
        ...
        function createGrid(){
            ...
            $(gridElm).jqxGrid({
                ...
                $(gridElm).jqxGrid('addgroup', 'grouptitle');
        ...
        function addGroup(data){
            ...
            function addColumnItem(c){
                ...
                li.find('a[data-action="edit"]').click(function(){
                    ...
                    frm.find('input[name="onchange"]').val(dataRecord.onchange);
                    frm.find('input[name="group_id"]').val(dataRecord.group_id);
                    frm.find('input[name="group_field"]').val(dataRecord.group_field);
                    frm.find('input[name="group_join"]').val(dataRecord.group_join);
                    frm.find('input[name="group_on"]').val(dataRecord.group_on);
                    frm.find('input[name="title"]').val(dataRecord.title);
                    frm.find('select[name="type"]').val(dataRecord.type).change();
                    frm.find('select[name="columndata"]').val(dataRecord.columndata||'').change();
            ...
            divElm.find('.add-sortable-item').click(function(){
                ...
                frm.find('input[name="onchange"]').val('');
                frm.find('input[name="group_id"]').val('');
                frm.find('input[name="group_field"]').val('');
                frm.find('input[name="group_join"]').val('');
                frm.find('input[name="group_on"]').val('');
                frm.find('input[name="title"]').val('');
                frm.find('select[name="type"]').val('string').change();
                frm.find('select[name="columndata"]').val('').change();
            ...
            Save: function(){
                ...
                var columns = $('#tabcontents-for-group>.tab-pane .sortable>li').get().map(function(c){
                    ...
                    if(
                        ...
                        // cdata.type == 'checkbox' ||
                        cdata.type == 'list' ||
                        cdata.type == 'server-dropdown' ||
                        cdata.type == 'server-multidropdown'
                        ){
                        delete cdata.client;
                        delete cdata.server;
                    }
                    if(
                        !(
                            ...
                            cdata.type == 'list' ||
                            cdata.type == 'parent' ||
                            cdata.type == 'treedropdown' ||
                            cdata.type == 'server-dropdown' ||
                            cdata.type == 'server-multidropdown' ||
                            cdata.type == 'group-server-dropdown' ||
                            cdata.type == 'group-server-multidropdown'
                        )
                        ){
                        delete cdata.sid;
                    }
                ...
                data.data.companyrequired = !!data.data.companyrequired;
                    ...
                    'buttons' : [{
                        ...
                        'click': function() {
                            ...
                            if (editingItem) {
                                ...
                                editingItem.attr('columndata', data.columndata);
                                editingItem.find('>div>span').html(data.title)
                                editingItem.attr('class','col-xs-'+data.col)
                            }else{
