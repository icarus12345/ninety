...
module.exports = function(Chart) {
	...
	Chart.layoutService = {
		...
		addBox: function(chart, item) {
			...
			item.fullWidth = item.fullWidth || true;
	...
	function drawPointLabels(scale) {
		...
		for (var i = getValueCount(scale) - 1; i >= 0; i--) {
			...
			if (pointLabelOpts.display) {
                ...
                if(pointLabelOpts.circular){
                    ctx.save();
                    ctx.beginPath();
                    scale._pointLabelSizes[i]
                    ctx.arc(pointLabelPosition.x, pointLabelPosition.y, pointLabelOpts.radius || 12, 0, 2 * Math.PI, false);
                    ctx.lineWidth = 1;
                    ctx.fillStyle = pointLabelOpts.backgroundColor;
                    ctx.strokeStyle = pointLabelOpts.borderColor;
                    ctx.fill();
                    ctx.stroke();
                    ctx.closePath();
                    ctx.restore();
                ...
                if(pointLabelOpts.circular){
				    ctx.textAlign = 'center';
                    pointLabelPosition.y=pointLabelPosition.y - scale._pointLabelSizes[i].h/2;
                } else {
