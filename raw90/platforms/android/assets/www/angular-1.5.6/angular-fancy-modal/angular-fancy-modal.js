  ...
  provider('$fancyModal', function() {
    ...
    this.$get = ['$controller', '$timeout', '$rootScope', '$injector', '$compile', '$http', '$templateCache', '$window', '$document', '$q',
      function($controller, $timeout, $rootScope, $injector, $compile, $http, $templateCache, $window, $document, $q) {
        ...
        function open(opts) {
            ...
            var table = angular.element('<div>').addClass('fancymodal-table');
            var cell = angular.element('<div>').addClass('fancymodal-cell');
            modal.bind('click touchend', closeByAction);
            table.append(cell);
            cell.append(content);
            modal.append(table);
    ...
    return {
      ...
      link: function(scope, element, attrs) {
        ...
        var options;
        // Trigger
        element.on(attrs.trigger || 'click touchend', function() {
